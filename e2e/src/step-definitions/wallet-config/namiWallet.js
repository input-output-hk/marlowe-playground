...
When(/^I authorize my nami wallet$/, async function () {
...
When(/^I configure my nami wallet$/, async function () {
...
When(/^I click the "([^"]*)" with "([^"]*)" text And sign the transaction with nami wallet$/, async function (role, name) {
    const { screen: { page }, globalStateManager } = this;
    let newPagePromise;
    newPagePromise = new Promise(resolve => page.context().once('page', resolve));
    await waitFor(async () => {
    ...
    await waitFor(async () => {
        ...
        const result = await locator.isVisible();
        if (result) {
            const password = process.env.NAMI_WALLET_PASSWORD;

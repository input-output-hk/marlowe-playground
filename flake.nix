{
  ...
  outputs = { self, nixpkgs, flake-utils, haskellNix, easy-purescript-nix, pre-commit-hooks, iohkNix, CHaP, std, nosys, ... }@inputs:
    let
      ...
      flake = flake-utils.lib.eachSystem supportedSystems (system:
        let
          easyPS =
            let
      ...
      mkJobs = path: {
        x86_64-linux = flake.packages.x86_64-linux.${path};
        x86_64-darwin = flake.packages.x86_64-darwin.${path};
      };
    in
    flake // {
      hydraJobs = {
        test-nix-fmt = mkJobs "test-nix-fmt";
        test-prettier = mkJobs "test-prettier";
        test-generated = mkJobs "test-generated";
        test-purs-tidy = mkJobs "test-purs-tidy";
        test-png = mkJobs "test-png";
        test-stylish-haskell = mkJobs "test-stylish-haskell";
        test-shell = mkJobs "test-shell";
      };
    };

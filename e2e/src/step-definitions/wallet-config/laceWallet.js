...
When(/^I authorize my lace wallet$/, async function () {
    const { screen: { page }, globalStateManager } = this;
    let newPagePromise;
...
When(/^I configure my lace wallet$/, async function () {
    const { screen: { page }, globalStateManager } = this;
...
When(/^I click the "([^"]*)" with "([^"]*)" text And sign the transaction with lace wallet$/, async function (role, name) {
    const { screen: { page }, globalStateManager } = this;
    let newPagePromise;
    newPagePromise = new Promise(resolve => page.context().once('page', resolve));
    await waitFor(async () => {
    ...
    await waitFor(async () => {
        ...
        const result = await locator.isVisible();
        if (result) {
            const password = process.env.LACE_WALLET_PASSWORD;

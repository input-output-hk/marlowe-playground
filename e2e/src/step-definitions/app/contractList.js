...
When(/^I can see "([^"]*)" contract id in the first row in the table$/, async function (contractName) {
...
When(/^I should see "([^"]*)" status of the "([^"]*)" contract$/, async function (expectedStatus, contractName) {
    const { page } = this.getScreen();
    const contractInfo = this.getContractInfo(contractName);
...
When(/^I start advancing "([^"]*)" contract$/, async function (contractName) {
    const { page } = this.getScreen();
    const contractInfo = this.getContractInfo(contractName);

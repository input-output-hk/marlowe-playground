...
Then(/^the "([^"]*)" should contain "([^"]*)" text$/, async function (role, name) {
...
Then(/^the "([^"]*)" input should contain "([^"]*)" value$/, async function (elementKey, expectedValue) {
...
Then(/^the "([^"]*)" for "([^"]*)" should contain "([^"]*)" text$/, async function (role, name, expectedText) {
    const { globalConfig } = this;
    const { page } = this.getScreen();
    await waitFor(async () => {
        const locator = page.getByRole(role, { name, exact: true });
...
Then(/^the "([^"]*)" with "([^"]*)" text should have "([^"]*)" class$/, async function (role, name, expectedClass) {
    const { page } = this.getScreen();
    await waitFor(async () => {
        const locator = page.getByRole(role, { name, exact: true });
...
Then('the {string} with {string} text should contain {string} text', async function (role, name, expectedText) {
    const { page } = this.getScreen();
    await waitFor(async () => {
        const locator = page.getByRole(role, { name, exact: true });
        const result = await locator.isVisible();
...
Then(/^the "([^"]*)" time should be (greater\s+than|less\s+than|equal\s+to) the "([^"]*)" time$/, async function (firstTimeLabel, operator, secondTimeLabel) {
...
Then(/^I expect the "([^"]*)" time to (increase|decrease) by "([^"]*)" minutes$/, async function (timeLabel, operation, increment) {
    const { globalStateManager } = this;
...
Then(/^I expect the "([^"]*)" time to match it's previous value$/, async function (timeLabel) {
    const { globalStateManager } = this;
    const timeName = `${timeLabel} time`;
    await waitFor(async () => {
        const timeValues = globalStateManager.get(timeName);

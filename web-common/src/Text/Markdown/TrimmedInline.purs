...
type FreshRenderer v m a = a -> Fresh (HTML v m)
...
renderInline i = case i of
  ...
  other -> pure $ HH.text
    ( prettyPrintMd
        ( SD.SlamDown $ L.singleton $ SD.Paragraph $ L.singleton other
            :: SD.SlamDown
        )
    )
...
markdownToHTML md = case parseMd md of
  Right (SD.SlamDown (Cons (SD.Paragraph singleLine) Nil)) -> map
    renderSlamDownInline
    (A.fromFoldable singleLine)
  _ -> [ HH.text md ]

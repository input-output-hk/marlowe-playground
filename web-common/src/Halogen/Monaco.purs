...
newtype CancelBindings = CancelBindings (Effect Unit)
...
_editor = prop (Proxy :: _ "editor")

_mCodeActionProvider :: Lens' State (Maybe CodeActionProvider)
_mCodeActionProvider = prop (Proxy :: _ "objects") <<< prop
  (Proxy :: _ "codeActionProvider")

_mCompletionItemProvider :: Lens' State (Maybe CompletionItemProvider)
_mCompletionItemProvider = prop (Proxy :: _ "objects") <<< prop
  (Proxy :: _ "completionItemProvider")

_disposables :: Lens' State (Array IDisposable)
_disposables = prop (Proxy :: _ "disposables")
...
data Query a
  ...
  | SetDeltaDecorations (Array String) (Array IModelDeltaDecoration)
      (Array String -> a)
...
monacoComponent
  :: forall m. MonadAff m => Settings m -> H.Component Query Unit Message m
...
handleAction settings Init = do
  ...
  case maybeElement of
    Just element -> do
      ...
      mDocumentFormattingEditProvider <- H.lift $
        settings.documentFormattingEditProvider
      liftEffect do
      ...
      disposables :: Array IDisposable <-
        liftEffect $ catMaybes
          <$> sequence
            [ for mMonarchTokensProvider $ Monaco.setMonarchTokensProvider
                monaco
                languageId
            , for mTokensProvider $ Monaco.setTokensProvider monaco languageId
            , for mHoverProvider $ Monaco.registerHoverProvider monaco
                languageId
            , for mCompletionItemProvider $
                Monaco.registerCompletionItemProvider monaco languageId
            , for mCodeActionProvider $ Monaco.registerCodeActionProvider monaco
                languageId
            , for mDocumentFormattingEditProvider $
                Monaco.registerDocumentFormattingEditProvider monaco languageId
            ]
      ...
      void $ H.subscribe $ elementResize ContentBox (const ResizeWorkspace)
        (HTMLElement.toElement element)
      H.raise EditorReady
...
withEditor
  :: forall input m a
   . MonadEffect m
  => (Editor -> HalogenM State Action input Message m a)
  -> HalogenM State Action input Message m (Maybe a)
...
handleQuery (SetText text next) = do
  withEditor \editor -> do
    ...
    when (text /= Monaco.getValue model) $ liftEffect $ Monaco.setValue model
      text
    pure next
...
handleQuery (GetDecorationRange decoratorId f) =
  runMaybeT do
    ...
    decoRange <- MaybeT $ liftEffect $ Monaco.getDecorationRange model
      decoratorId
    pure $ f decoRange
...
handleQuery (SetKeyBindings bindings next) =
  withEditor \editor -> do
    ...
    newDeactivateBindings <- liftEffect $ replaceKeyBindings bindings editor
      deactivateBindings
...
replaceKeyBindings
  :: KeyBindings -> Editor -> CancelBindings -> Effect CancelBindings
